...
public class SampleController {
	...
	public ModelAndView BoardList(@RequestParam(defaultValue = "title") String searchOption,
		 ...
		 * list); // list map.put("count", count); // 레코드의 갯수 map.put("searchOption",
		 * searchOption); // 검색옵션 map.put("keyword", keyword); // 검색키워드
		 */
	...
	public String writeBoard(@RequestParam MultipartFile[] file, @ModelAttribute BoardVO vo) throws Exception { // 210325
																												// 파일 다중
																												// 업로드
		...
		for (int i = 0; i < file.length; i++) {
			...
			new ResponseEntity<String>(uploadFile(uploadPath, file[i].getOriginalFilename(), file[i].getBytes()),
					HttpStatus.OK);
		}
	...
	@RequestMapping("sample/downloadFile.do")
	public void downloadFile(String fullname, Model model, HttpServletResponse response) throws Exception {
		// model.addAttribute("fileInfo", sampleService.selectFileInfo(fullname));
		byte fileByte[] = FileUtils.readFileToByteArray(new File("C:\\upload\\" + fullname));
		response.setContentType("application/octet-stream");
		response.setContentLength(fileByte.length);
		response.setHeader("Content-Disposition",
				"attachment; fileName=\"" + URLEncoder.encode(fullname, "UTF-8") + "\";");
